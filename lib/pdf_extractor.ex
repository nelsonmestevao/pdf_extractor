defmodule PdfExtractor do
  ...
  This is the main function for text extraction. It supports extracting from single pages,
  multiple pages, and specific areas within pages.

  ## Parameters

  - `file_path` - Path to the PDF file (string)
  - `page_numbers` - Page(s) to extract from (integer, list of integers, or empty list for all pages)
  - `areas` - Optional bounding boxes for area-based extraction (map)
  ...
  ## Examples
      ...
      ...>   10 => {0, 0, 612, 190}
      ...> })
      %{2 => "Introdução – Nota do tradutor", 8 => "I. Sobre aproveitar o tempo", 10 => "II. Sobre a falta de foco na Leitura"}
  ...
  ## Examples
      ...
      iex> binary = File.read!("priv/fixtures/fatura.pdf")
      iex> result = PdfExtractor.extract_text_from_binary(binary)
      iex> is_map(result) and map_size(result) > 0
      true

      # Extract from specific pages using binary
      iex> binary = File.read!("priv/fixtures/fatura.pdf")
      iex> result = PdfExtractor.extract_text_from_binary(binary, [0])
      iex> is_map(result) and Map.has_key?(result, 0)
      true
  ...
  @doc """
  Extracts metadata from PDF binary data.

  Similar to `extract_metadata/1` but works with PDF data in memory instead of files.
  ...
  Returns a map with the same format as `extract_metadata/1`.

  ## Examples

      # Extract metadata from PDF binary data
      iex> binary = File.read!("priv/fixtures/fatura.pdf")
      iex> metadata = PdfExtractor.extract_metadata_from_binary(binary)
      iex> is_map(metadata)
      true
